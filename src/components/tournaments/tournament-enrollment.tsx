...
export function TournamentEnrollment({
  ...
  const [teamId, setTeamId] = React.useState(available[0]?.id ?? "");
  const [error, setError] = React.useState<string | null>(null);
  const [pending, startTransition] = React.useTransition();

  React.useEffect(() => {
    if (!available.some((t) => t.id === teamId)) {
      setTeamId(available[0]?.id ?? "");
    }
  }, [available, teamId]);
    ...
    <div className="space-y-4">
      ...
      {enrolled.length === 0 ? (
        <div className="border border-dashed border-[#D0D5DB] bg-white px-6 py-10 text-center">
          <p className="font-medium text-[#0A0A0A]">No teams enrolled</p>
          <p className="mt-1 text-sm text-[#5C6570]">
            {canManageStaff
              ? "Pick a team above to add it to this tournament."
              : "League staff will enroll teams when registration opens."}
          </p>
        </div>
      ) : (
